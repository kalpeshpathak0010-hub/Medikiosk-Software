...
export const UnauthorizedAccessView: React.FC<UnauthorizedAccessViewProps> = ({
  ...
  const { currentUser, loginStaffWithEmail, registerStaffAccount } = useAuth();
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 pb-6 mb-6 border-b border-slate-200/80 text-center sm:text-left">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs sm:text-sm text-slate-600 mt-1 font-medium leading-relaxed">
              You are currently viewing as <strong className="text-slate-900 font-bold">{currentUser.name}</strong> (Role: <span className="px-2 py-0.5 rounded-md bg-slate-200 font-mono font-bold text-slate-800 text-xs">{currentUser.role}</span>).
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-xs font-black uppercase tracking-wider text-slate-200">
                {authMode === 'signin' ? 'Firebase Staff Sign In' : 'Create Staff Profile'}
              </h4>
            </div>
