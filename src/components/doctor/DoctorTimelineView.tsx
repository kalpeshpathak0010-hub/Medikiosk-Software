      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-6 border-b border-slate-200/80">
        <div>
          ...
          <p className="text-xs text-slate-600 font-medium">
            Patient: <strong className="text-slate-900 font-bold">{patientName}</strong>{' '}
            {patientAge && `(${patientAge} / ${patientGender})`} • Synthesized from OCR documents & previous records
          </p>
        </div>
      </div>

      {events.length === 0 ? (
        <div className="bg-white/80 rounded-2xl border border-slate-200 p-12 text-center max-w-md mx-auto my-8 shadow-xs">
          <Clock className="w-10 h-10 text-slate-400 mx-auto mb-3" />
          <h3 className="text-base font-bold text-slate-900 mb-1">No Timeline Events Logged</h3>
          <p className="text-xs text-slate-500">
            Past clinical documents uploaded during registration or doctor review will appear here as a chronological longitudinal timeline.
          </p>
        </div>
      ) : (
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="relative pl-6 sm:pl-8 border-l-2 border-slate-300 space-y-6">
              {events.map((evt) => {
                ...
                const isSelected = evt.id === (selectedEvent?.id || selectedEventId);
